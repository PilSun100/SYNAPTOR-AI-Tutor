...
- 자료 언어와 답변 언어가 달라도 의미 기준으로 평가하세요.
- 영어 강의자료에 한글 답변을 제출해도 핵심 개념, 관계, 원인-결과 설명이 맞으면 정답 또는 부분 정답으로 인정하세요.
- 단순 키워드 일치보다 개념 의미, 관계, 원인-결과 설명을 우선하세요.
- 핵심 키워드를 그대로 쓰지 않아도 같은 의미의 다른 언어 표현이면 부분 점수를 주세요.
- 의미 없는 답변, 너무 짧은 답변, 반복 문자, 질문과 무관한 답변은 correctness_score를 0.0으로 처리하세요.
...
힌트 레벨:
1 = 정답 방향을 스스로 떠올리게 하는 질문
2 = evidence 기반 관련 개념 설명
3 = 답변을 시작할 수 있는 문장 틀
4 = 핵심 키워드 목록
5 = 거의 정답에 가까운 구조. 단, 그대로 베끼지 말고 자기 말로 설명하도록 유도
...
- 빈칸 마스킹이나 자모 단서만 던지는 힌트는 제공하지 마세요.
- 레벨 1은 반드시 질문 형태로 시작하고, 레벨이 올라갈수록 더 구체적인 이해 발판을 주세요.
